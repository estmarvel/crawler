...
def classify_title(raw_title):
    ...
    'skip'      控制价，直接跳过
    'terminate' 终止/废标/撤销
    'amendment' 变更/更正/澄清/延期
    'result'    中标结果公示
    'candidate' 中标候选人公示
    'notice'    招标公告/预审公告
    'plan'      招标计划（无已知后缀）
    'unknown'   未识别，按所在频道默认类型处理
    """
    # skip优先，避免"招标控制价变更"被误匹配为变更公告
    ...
    for kw in AMENDMENT_KEYWORDS:
        if kw in raw_title:
            return "amendment"
    # result在candidate前，避免"中标结果公示"误匹配candidate
...
def step3_amendment(df):
    ...
    for item in items:
        ...
        if type_ == "skip":
            ...
            # amendment / unknown / plan → 当变更公告处理

...
# ==================== 增量总表配置 ====================
# 每天定时运行都读写这个固定总表，不再每天生成一个新的 CSV。
# 程序启动时先读取该文件，新增项目插入，已有项目更新，保存时仍保留历史数据。
CSV_PATH = os.path.join(OUTPUT_DIR, "山西.csv")

# 每天日志仍然按日期分开，方便排查每天运行情况。
...
RUN_LOG_PATH    = os.path.join(LOG_DIR, f"{TODAY_STR}_run.log")

CRAWL_DAYS = 1   # 每天定时运行只爬当天公告，实现增量更新

SESSION = requests.Session()

# ==================== IP代理池配置（只用于山西平台请求，不用于AI请求） ====================
USE_PROXY = True

# False：代理池为空时不使用服务器真实IP直连，避免服务器IP暴露给目标站。
DIRECT_FALLBACK_WHEN_PROXY_EMPTY = False

# True：校验 HTTPS 证书；如果代理导致 SSLError，可临时改为 False。
VERIFY_SSL = True

# 天启短效代理建议用5分钟，比3分钟更稳；坏代理失败1次即剔除。
...
# ==================== AI 配置 ====================
# 不建议把AI Key硬编码到源码里。运行前执行：export DMX_API_KEY='你的真实AI接口Key'
AI_API_KEY = os.getenv("DMX_API_KEY", "").strip()
AI_MODEL   = "gpt-4o-mini"
AI_BASE_URL = "https://vip.dmxapi.com/v1"

# AI请求不走短效代理池；这里只做限速和退避，防止API侧限流。
...
def get_ai_client() -> OpenAI:
    if not AI_API_KEY:
        raise RuntimeError("请先设置环境变量 DMX_API_KEY，例如：export DMX_API_KEY='sk-你的真实AI接口Key'")
    return OpenAI(api_key=AI_API_KEY, base_url=AI_BASE_URL)


def sleep_before_ai_call() -> None:
    """AI接口限速：AI请求不走短效代理池，只控制请求频率。"""
...
def ai_retry_sleep(attempt: int) -> None:
    wait_seconds = min(AI_RETRY_BASE_SECONDS * (2 ** attempt), AI_RETRY_MAX_SECONDS)
...
def get_proxy_for_request():
    """从代理池获取一个 requests 可直接使用的 proxies 字典。"""
    if not USE_PROXY:
        return None
...
def should_mark_bad(resp=None, exc=None):
    ...
    if exc is not None:
        return isinstance(
            exc,
            (ProxyError, ConnectTimeout, ReadTimeout, SSLError, RequestException),
        )
    if resp is None:
        return True
...
def get(url, retries=3):
    ...
    for i in range(retries):
        ...
        try:
            ...
            print(f"  [GET异常{i+1}/{retries}] status={resp.status_code} proxy={proxy} | {url}")
            ...
            print(f"  [GET失败{i+1}/{retries}] {type(e).__name__}: {e} | proxy={proxy} | {url}")
...
def post_req(url, data, retries=3, headers=None):
    ...
    for i in range(retries):
        ...
        try:
            ...
            print(f"  [POST异常{i+1}/{retries}] status={resp.status_code} proxy={proxy} | {url}")
            if USE_PROXY and proxy and should_mark_bad(resp=resp):
                PROXY_POOL.mark_bad(proxy)
            ...
            print(f"  [POST失败{i+1}/{retries}] {type(e).__name__}: {e} | proxy={proxy} | {url}")
            if USE_PROXY and proxy and should_mark_bad(exc=e):
                PROXY_POOL.mark_bad(proxy)
...
def save_csv(df):
    """
    保存增量总表。
    注意：这里虽然会重写 CSV 文件，但 df 是先从旧总表 load_csv() 读入后的完整数据，
    所以不会覆盖丢失历史数据。写入时先写 .tmp，再原子替换，降低中途异常损坏文件的风险。
    """
    for col in COLUMNS:
        if col not in df.columns:
            df[col] = "暂无"
    df = df[COLUMNS].fillna("暂无")
...
def main():
    print("=" * 60)
    print("山西省公共资源交易平台爬虫 v3.6（正则优先 + AI兜底 + IP代理池）")
    ...
    print("初始化Session...")
    try:
        init_resp = get("https://prec.sxzwfw.gov.cn/jyxx/index.jhtml", retries=3)
        if init_resp:
            print("Session初始化成功")
        else:
            print("Session初始化失败，但继续尝试后续列表请求")
    except Exception as e:
    ...
    if USE_PROXY:
        try:
            print(f"代理池状态: {PROXY_POOL.stats()}")
        except Exception as e:
            print(f"代理池状态读取失败: {e}")

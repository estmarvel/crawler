...
# ========== AI配置 ==========
AI_API_KEY = os.getenv("DMX_API_KEY", "")
...
# ============================================================
# 代理池配置：只用于山西公共资源交易平台请求，不用于 AI 请求
# ============================================================

USE_PROXY = True

# False：代理池为空时不降级直连，避免服务器真实 IP 访问目标站点。
# True ：代理池为空时允许直连，稳定性更高，但会暴露服务器真实 IP。
DIRECT_FALLBACK_WHEN_PROXY_EMPTY = False

# 天启代理池默认参数：num=10, time=3, port=2
# 等价于每次提取 10 个、3 分钟、HTTPS 代理。
PROXY_POOL = ProxyPool(num=10, time=3, port=2)

# HTTPS 证书校验。正常建议 True；如果代理导致 SSL 报错，可临时改为 False。
VERIFY_SSL = True

# 这些状态码通常表示代理不可用、过期、未授权、被限流或目标站拒绝。
...
# ============================================================
# AI 请求限速配置：AI 不走代理池，防止 API Key 被限流/风控
# ============================================================
...
def sleep_before_ai_call() -> None:
    """
    AI 请求限速：AI 不走代理池，但需要控制调用频率，避免 429 或 API Key 风控。
    """
...
def get_proxy_for_request() -> Optional[Dict[str, str]]:
    """
    从代理池取一个代理。
    USE_PROXY=False 时返回 None。
    代理池为空且不允许直连时，抛出 ProxyPoolEmptyError。
    """
    if not USE_PROXY:
        return None
    ...
    try:
        ...
        if DIRECT_FALLBACK_WHEN_PROXY_EMPTY:
            print("  [代理] 允许直连降级，本次将使用服务器真实 IP")
            return None
        raise


def should_mark_bad(resp: Optional[requests.Response] = None, exc: Optional[Exception] = None) -> bool:
    """
    判断当前代理是否应该标记为坏代理。
    """
    if exc is not None:
...
def get(url: str, retries: int = 3) -> Optional[requests.Response]:
    """
    GET 请求统一入口：详情页、Session 初始化等访问目标站点的请求都走这里。
    """
    for i in range(retries):
        ...
        try:
            ...
            print(
                f"  [GET异常 {i + 1}/{retries}] "
                f"status={resp.status_code} proxy={proxy} {url}"
            )
            ...
            print(
                f"  [GET失败 {i + 1}/{retries}] "
                f"{type(e).__name__}: {e} | proxy={proxy} | {url}"
            )
...
def get_ai_client() -> OpenAI:
    if not AI_API_KEY:
        raise RuntimeError(
            "未检测到 AI Key。请先设置环境变量：export DMX_API_KEY='你的API_KEY'"
        )

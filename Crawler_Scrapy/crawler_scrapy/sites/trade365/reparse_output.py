"""使用已保存的 HTML 溯源包原地重算中招联合山西 JSON/CSV 字段。"""
...
def reparse(output_root: Path) -> int:
    ...
    for path in paths:
        ...
        for row in rows:
            ...
            row["附件"] = canonicalize_attachment_list(parsed.attachments)
            ...
            if isinstance(field_meta, dict):
                ...
                field_meta["validation_warnings"] = list(parsed.validation_warnings)
            total += 1
            changed += before != {
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="从HTML溯源包重算中招联合山西结果")
    parser.add_argument("--output-root", type=Path, default=Path("new_output"))
    args = parser.parse_args(argv)
    output_root = args.output_root.expanduser().resolve()
    lock_path = output_root / "trade365" / "state" / "resumable.lock"
    lock_path.parent.mkdir(parents=True, exist_ok=True)
    with lock_path.open("a+") as lock_file:
        try:
            fcntl.flock(lock_file.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError:
            print("trade365 正在运行，拒绝修改JSON/CSV", file=sys.stderr)
            return 5

...
# 出口模式由具体 Spider 读取。华新默认使用固定认证代理，只允许 static/tianqi；
# direct 会在 Spider 初始化阶段被拒绝，保证任何情况下都不使用服务器公网 IP。
CRAWLER_OUTBOUND_MODE = "static"

# 固定代理沿用此前的保守限速参数；变量名保留 DIRECT_ 前缀以兼容已有部署命令，
# 但华新已禁止 direct 出口模式。
DIRECT_CONCURRENT_REQUESTS = 2
DIRECT_CONCURRENT_REQUESTS_PER_DOMAIN = 1
DIRECT_DOWNLOAD_DELAY = 3.0
DIRECT_AUTOTHROTTLE_START_DELAY = 3.0
DIRECT_AUTOTHROTTLE_MAX_DELAY = 60.0
DIRECT_AUTOTHROTTLE_TARGET_CONCURRENCY = 0.5
DIRECT_RETRY_TIMES = 1
DIRECT_MAX_RESPONSES_PER_RUN = 300

# 403/429 第一次即退避；同一域名连续 2 次或本次任务累计 4 次时主动停爬。
DIRECT_GUARD_HTTP_CODES = [403, 429]
DIRECT_GUARD_CONSECUTIVE_LIMIT = 2
DIRECT_GUARD_TOTAL_LIMIT = 4
DIRECT_GUARD_BASE_BACKOFF = 30.0
DIRECT_GUARD_MAX_BACKOFF = 300.0

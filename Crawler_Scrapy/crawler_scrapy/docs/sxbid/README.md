...
UUID 保持对应。若 PDF 没有文字层或下载失败，记录保存为 `PARTIAL`，不会伪造缺失字段。
...
- `_trace.rawHtml/rawText/payload/responseMetadata`；
- HTML 快照路径和 SHA256；
- 源站名称、栏目、地区、项目类型、项目链编号及 PDF 地址。

当前规则版本为 `sxbid-v2-verified-html-pdf`。第二轮真实样本复核补充了以下处理：
...
```bash
./run_sxbid.sh --phase all --days 180
```
